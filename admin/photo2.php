  ...
  <form method="post" action="./api/editphoto_api.php">
    <table width="98%">
      <tbody>
        ...
        $sql="select count(*) from `photo` where `id`='$id'";
        ...
        $sql1="SELECT `pid`, `id`, `file`, `text`, `sh` 
     ...
     <div>
        <?php
            if(($p-1)>0){
              echo "<a href='admin.php?do=photo&p=".($p-1)."' style='text-decoration:none'> < </a>";
            }

            for($i=1;$i<=$pages;$i++){
              $fontSize=($i==$p)?"24px":"16px";
              echo "<a href='admin.php?do=photo&p=$i' style='font-size:$fontSize;text-decoration:none'> ".$i." </a>";
            }
